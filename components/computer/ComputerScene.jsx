...
function SceneContent(props){const controls=useRef();const {gl}=useThree();useEffect(()=>{const lost=()=>props.onContextLost();gl.domElement.addEventListener('webglcontextlost',lost);return()=>gl.domElement.removeEventListener('webglcontextlost',lost);},[gl,props.onContextLost]);return <><color attach="background" args={['#edf0ed']}/><ambientLight intensity={1.35}/><hemisphereLight args={['#ffffff','#6c7e73',1.5]}/><directionalLight position={[3,7,6]} intensity={2.5} castShadow shadow-mapSize={[1024,1024]} shadow-camera-left={-5} shadow-camera-right={5} shadow-camera-top={5} shadow-camera-bottom={-5} shadow-bias={-.001}/><directionalLight position={[-4,2,-3]} intensity={1.6}/><ComputerModel {...props}/><mesh rotation={[-Math.PI/2,0,0]} position={[0,-2.85,0]} receiveShadow><planeGeometry args={[200,200]}/><shadowMaterial transparent opacity={.11}/></mesh><OrbitControls ref={controls} makeDefault enableDamping={!props.reduced} dampingFactor={.09} minDistance={3.5} maxDistance={25} minPolarAngle={.2} maxPolarAngle={Math.PI*.84} enablePan={false} zoomSpeed={.75} rotateSpeed={.65}/><CameraController controls={controls} {...props}/></>;}
function WebGLFallback(){return <div className="model-fallback">Twoja przeglądarka nie udostępnia WebGL. Skorzystaj z Listy elementów poniżej.</div>;}
export default function ComputerScene(props){const [supported]=useState(()=>{try{const canvas=document.createElement('canvas');const context=canvas.getContext('webgl2');if(!context)return false;context.getExtension('WEBGL_lose_context')?.loseContext();return true;}catch{return false;}});useEffect(()=>{if(!supported)props.onContextLost();},[supported]);if(!supported)return <WebGLFallback/>;return <Canvas aria-label="Interaktywny model 3D komputera. Alternatywna obsługa dostępna przez Listę elementów." role="img" shadows frameloop="demand" dpr={[1,1.5]} camera={{position:[4,3,10],fov:38,near:.1,far:100}} gl={{antialias:true,alpha:false,powerPreference:'default'}} fallback={<WebGLFallback onError={props.onContextLost}/>} onCreated={({gl})=>{props.onReady();}}><Suspense fallback={null}><SceneContent {...props}/></Suspense></Canvas>;}
