...
export default function ComputerModel({isolated,exploded,selected,onSelect,reduced,slot,onSlot}){return <group>
 ...
 {motherboardSlots.filter(()=>!isolated||selected==='motherboard').map(s=><group key={s.id} position={s.position} onClick={e=>{if(e.delta>6)return;e.stopPropagation();onSlot(s.id);}} userData={{slotId:s.id}}><MaterialState.Provider value={{selected:slot===s.id,dim:!!selected&&selected!=='motherboard'}}><Box size={s.size} color={slot===s.id?'#608471':'#242f33'}/></MaterialState.Provider></group>)}
 </group>;}
